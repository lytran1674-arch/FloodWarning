...
// //   const [, setSelectedAreaLabel] = useState("");
...
//   // Địa chỉ nhập tay -> suy ra lat/lon
//   const [address, setAddress] = useState("")
//   const [geocoding, setGeocoding] = useState(false)
//   const [geocodeError, setGeocodeError] = useState<string | null>(null)

//   const [lat, setLat] = useState<number | null>(null)
//   const [lon, setLng] = useState<number | null>(null)
...
//     } catch (error) {
//       console.error("Geocode error", error)
//       setGeocodeError("Lỗi khi tra cứu địa chỉ")
//     } finally {
//       setGeocoding(false)
//     }
//   }

//   // debounce: tự động geocode sau khi người dùng ngừng gõ 800ms
...
//   // Người dùng bấm vào bản đồ để tự chỉnh vị trí sau khi geocode
...
//     if (!lat || !lon) {
//       toast.warning(
//         "Chưa xác định được tọa độ từ địa chỉ, vui lòng kiểm tra lại địa chỉ hoặc bấm chọn vị trí trên bản đồ"
//       )
//       return
//     }
...
//         if (!sosId) {
//           toast.error("Không tìm thấy ID SOS để cập nhật")
//           return
//         }

//         await updateSoS(sosId, payload)
...
//       navigate("/success")
//     } catch (err: any) {
//       console.error("CREATE / UPDATE SOS ERROR:", err)
...
//       toast.error(
//         err?.response?.data?.message || "Gửi yêu cầu SOS thất bại"
//       )
//     }
//   }
...
//           <Label icon={Map} className="text-red-600 font-semibold text-sm">
//             Vị trí
//           </Label>
...
//                 placeholder="Nhập địa chỉ, ví dụ: 123 Nguyễn Huệ, Quận 1, TP.HCM"
//                 className="w-full border rounded-lg pl-2.5 pr-8 py-1.5 text-xs"
//               />
//               {geocoding && (
//                 <Loader className="w-3.5 h-3.5 animate-spin text-red-500 absolute right-2 top-1/2 -translate-y-1/2" />
//               )}
//             </div>
...
//             {lat && lon && !geocodeError && (
//               <p className="text-[11px] text-slate-400 flex items-center gap-1">
//                 <MapPin className="w-3 h-3" />
//                 Đã xác định tọa độ: {lat.toFixed(5)}, {lon.toFixed(5)} — có thể
//                 bấm vào bản đồ để chỉnh lại
//               </p>
//             )}
//           </div>
...
//             <p className="text-red-600 font-semibold text-sm">Tình trạng</p>
//           </div>
...
//         {/* PHONE */}
//         <section className="space-y-2">
//           <Label icon={PhoneCall} className="text-red-600 font-semibold text-sm">
//             Số điện thoại
//           </Label>
...
//         {/* MÔ TẢ */}
//         <section className="space-y-2">
//           <Label icon={FileText} className="text-red-600 font-semibold text-sm">
//             Mô tả tình trạng
//           </Label>
...
//           className="w-full bg-red-600 text-white py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-60"
//         >
//           {loading ? (
//             <Loader className="w-4 h-4 animate-spin" />
//           ) : (
