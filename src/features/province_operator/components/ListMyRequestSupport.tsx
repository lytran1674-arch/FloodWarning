...
export default function ListMyRequestSupport() {
  const {
    ...
    assignGroupToRequest, // ⚠️ cần thêm hàm này trong useRequestSupport (xem gợi ý bên dưới)
  } = useRequestSupport();
 ...
 const {
  ...
  const openAssignModal = (item: { id: string; supportType: string }) => {
...
const handleSubmitAssign = async () => {
  ...
  try {
    setAssigning(true);
    setAssignError(null);
    const success = await assignGroupToRequest(assignTarget.id, groupId, note.trim());
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    SOS ID: <span className="font-mono">{item.sosId}</span>
                  </p>

                  <p className="mt-1 text-sm text-gray-500">
                    Người yêu cầu: {item.requestByName}
                  </p>
              ...
              {item.provinceResponse && (
                <div className="mt-3 rounded-lg bg-blue-50 p-3 text-sm text-blue-700">
                  <span className="font-medium">Phản hồi tỉnh:</span>{" "}
                  {item.provinceResponse}
                </div>
              )}
              ...
              <div className="mt-4 flex justify-end gap-2">
                {/* Chỉ cho phân công khi tỉnh đã duyệt và chưa gán đội */}
                {item.status === "APPROVED" && !item.assignedTeamName && (
                  <button
                    onClick={() =>
                      openAssignModal({
                        id: item.id,
                ...
                <button
                  onClick={() => navigate(`/support-request/${item.id}`)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-lg">
            ...
            <label className="mb-1 block text-sm font-medium text-gray-700">
              Mã nhóm cứu hộ (groupId)
            </label>
            {/* TODO: thay input này bằng <select> nếu có API danh sách nhóm của đội */}
            <label className="mb-1 block text-sm font-medium text-gray-700">
  Nhóm cứu hộ
</label>
<select
