...
export const Home = () => {
  ...
  // Hook GPS – chỉ trả về gpsArea, parentArea, polygons, toạ độ, loading
  const {
    gpsArea,
    parentArea,
    polygons,
  ...
  // Xác định parentId: ưu tiên parentArea (quận/huyện), nếu không có thì null
  const effectiveParentId = useMemo(() => parentArea?.id ?? null, [parentArea])

  // Tên hiển thị trên map: lấy từ parentArea hoặc thông báo chung
  const effectiveParentName = useMemo(() => {
    return parentArea?.tenkhuvuc ?? "Khu vực của bạn"
  }, [parentArea])

  // Lấy danh sách khu vực + risk theo lead, dùng parentId
  const { areas: riskAreas, loading: loadingMap } = useProvinceMap(
    effectiveParentId,
    lead
  )

  // Ghép polygon (từ GPS) vào các area
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex gap-4 items-start">
        <GeoMap
          ...
          userAreaId={gpsArea?.id}          // vị trí chính xác của user
      ...
      {/* Hiển thị khu vực của bạn (dùng gpsArea) */}
      {gpsArea && (
