...
// ✅ Page không nhận props của Card nữa
export const RequestListPage = () => {
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true)
      setError(null)
      const res  = await axiosClient.get("/sos-request/my-sos")
      const data = res.data?.result?.content ?? res.data?.content ?? []
  ...
  // Scroll đến card được highlight
  useEffect(() => {
  ...
  // ── Cancel flow ──
  // Card gọi onCancel(id) → Page mở confirm modal
      ...
      {existingBannerVisible && (
        <div className="flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2.5 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-amber-800">Bạn đang có yêu cầu SOS chưa hoàn thành</p>
      ...
      <div className="flex flex-col gap-3 flex-1">
        ...
        {error && !loading && (
          <div className="text-center py-16">
            ...
            <button onClick={fetchRequests} className="text-xs text-red-600 underline hover:text-red-700">
              Thử lại
            </button>
          </div>
        )}
        ...
        {!loading && !error && filtered.map(req => (
          ...
          >
            {/* ✅ Nút hủy nằm trong Card — truyền callback onCancel */}
            <SosRequestCard
              request={req}
              highlight={req.id === highlightId}
              sosData={req.id === highlightId ? sosData : undefined}
              onCancel={handleRequestCancel}          // ← Page xử lý confirm
              cancelLoading={cancelTargetId === req.id && cancelLoading}
            />
          </div>
        ))}
      </div>
