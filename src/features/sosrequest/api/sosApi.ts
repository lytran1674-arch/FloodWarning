...
import type { CancelResponse } from "@/features/sosrequest-anonymous/types/sosanonymousType"
import type { AssignSos, AssignSosResult, DetailSos, DetailSoSCitizen, SentedSupportRequest, SoSRequest, SoSResponse } from "../types/sosType"
import { axiosClient, publicApi } from "@/api/axiosClient"
...
{sodt, clientDeviceId} )
  ...
  // ĐÃ SỬA: trước đây khai báo Promise<string> — đây chính là nguồn gốc lỗi TS.
  // response.data.result giờ là { assignmentId, callTask }, không còn là string đơn thuần
  // (BE đổi theo spec "Cập nhật luồng Assign Rescue Group" Bước 1).
  async postassigment(data: AssignSos): Promise<AssignSosResult> {
    const response = await axiosClient.post("/sos-assignment", data)
    return response.data.result
  },
